...
public interface ProductDao extends BaseMapper<Product> {
    ...
    @Select("<script>" +
            "SELECT p.id,p.pd_name,p.price,p.cost,p.yield,p.period_min,p.unit,p.period_max,p.height,p.characters,p.num,p.is_show,t.type_name AS type FROM product AS p LEFT JOIN type AS t ON t.id = p.type_id WHERE 1=1 " +
    ...
    @Insert("<script>" +
            ...
            "type_id," +
            "</when>" +
            "id,pd_name,price,cost,num,period_min,period_max,unit,yield,height,characters,create_time) " +
            "VALUE(" +
            "<when test=\" type !='' and type!=null\">" +
            "#{type}," +
            "</when>" +
            "#{id},#{pdName},#{price},#{cost},#{num},#{periodMin},#{periodMax},#{unit},#{yield},#{height},#{characters},NOW())" +
            "</script>")
