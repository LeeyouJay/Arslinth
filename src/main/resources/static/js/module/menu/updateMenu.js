			...
			success: function(data) {
				if (data.code === 200) {
					...
					for (var i = 0; i < data.data.menuLevel.length; i++) {
						...
						if (dataSelcet == $("#level").val()) {
							level += " <option value='" + dataSelcet + "' selected>" + dataSelcet + "</option>"
						} else {
			...
			success: function(data) {
				if (data.code === 200) {
					...
					$.each(menuNames, function(index, item) {
						if (item.id == $("#getparentId").val()) {
							level += " <option value='" + item.id + "' selected>" + item.menuName + "</option>"
						} else {
